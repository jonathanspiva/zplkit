...
```bash
# Test harness: renders ZPL → PNG for visual inspection
npm install zpl-renderer-js

# Swift test generates ZPL, writes to file
# Node script renders to PNG
# Snapshot comparison or manual review
```

**Test workflow:**
1. Swift test generates ZPL string
2. Writes to `test_output/label_name.zpl`
3. Node script batch-renders all `.zpl` files to `.png`
4. Visual diff against baseline images
